...
class TestAddResourceArgs:
    ...
    @pytest.mark.asyncio
    async def test_rejects_git_local_watch(
        self,
        resource_service: ResourceService,
        request_context: RequestContext,
    ):
        with pytest.raises(InvalidArgumentError, match="one-time sources"):
            await resource_service.add_resource(
                path="/tmp/repo.zip",
                ctx=request_context,
                to="viking://resources/local-git/test-repo/main",
    ...
    @pytest.mark.asyncio
    async def test_rejects_git_local_without_temp_file_id(
        self,
        resource_service: ResourceService,
        request_context: RequestContext,
    ):
        with pytest.raises(InvalidArgumentError, match="temp_file_id"):
            await resource_service.add_resource(
                path="/tmp/repo.zip",
                ctx=request_context,
                to="viking://resources/local-git/test-repo/main",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ({"add_type": "git_local"}, "cannot be combined with add_type"),
